...
这五类职责不是只保存在架构图。运行 `bash scripts/build_n31_agent_trace.sh` 会从19个真实成果重建严格Schema追踪，登记13个受控工具、14次证据绑定调用和5次交接，并逐项核对成果大小与SHA-256。追踪必须包含 `Verifier → Revision → Verifier` 复检环；任何越权工具、未知成果、哈希漂移、路径或凭证模式都会被拒绝。Web通过 `/api/n31/agents` 和“Agent工具链”面板展示同一份追踪。
...
每次八阶段Gold现场运行还会在私有运行清单中记录阶段耗时、CPU用户/系统时间、进程累计峰值RSS和阶段输出字节。耗时与CPU是阶段增量，RSS是进程累计峰值，不能误解为单阶段独占内存；字段含义由 `PROCESS_CUMULATIVE_PEAK_AND_STAGE_DELTAS` 固定。旧清单缺少这些字段时仍可读取，新运行和路演验收必须完整记录。
