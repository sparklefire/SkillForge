...
```bash
bash scripts/check_public_checkout.sh
# 开发中只验证当前HEAD归档，不产生正式通过结论
bash scripts/check_public_checkout.sh --allow-dirty
```

检查器只使用 `git archive HEAD` 中的精确跟踪文件，拒绝 `.env`、私有输入、运行输出、链接、路径穿越和本机实际密钥值；随后在临时目录中以 `pip --no-index --no-deps --no-build-isolation` 安装包，阻断Socket连接，并重新验证Gold严重问题5→0、Web接口、20成员技术包和路演状态。临时检出会自动删除，私有QA写入Git忽略的 `outputs/reproducibility/`，权限为目录700、文件600。正式状态还要求 `main` 工作树干净且当前提交等于 `origin/main`；无Git的DGX部署预检只能明确跳过，不能替代公开提交复现。
...
预检会运行全量测试并执行20项固定检查，核对项目身份、12份说明文档、赛事征文事实绑定、18项成果、Git工作树、公开提交纯净检出、跟踪文件边界、`.env`忽略与600权限、本地密钥值泄漏、80秒成片观看、178秒最终录屏观看、彩排、官方规则、团队记录、官方表单材料包及人工确认有效性。报告和确认目录均由Git忽略；报告只写门禁汇总，不记录密钥值、确认人、规则结论、来源网址、说明、私有备注或证据路径。只有 `READY_FOR_SUBMISSION` 返回0；`NOT_READY`返回1，`DEVELOPMENT_CHECK`或 `READY_WITH_HUMAN_GATES` 返回2。开发中可显式使用 `--allow-dirty`，但不能得到正式提交结论；正式预检要求当前提交已有来源绑定的 `PASSED` 纯净检出报告。
...
- 本机当前通过378项自动测试；公开HEAD纯净检出可在无 `.env`、私有输入、历史输出和网络请求的临时环境中复现Gold 5→0、Web 200、20成员技术包与路演状态。DGX上一部署批次通过373项，新增公开检出检查需在同步后复验；无Git部署预检只会明确跳过该项，不能替代公开仓库验证。来源绑定赛事征文、官方表单材料包、最终录屏完整观看证据链、引导式人工观看/彩排和11阶段收尾链已在DGX Python 3.12复验，部署副本保持表单/录屏审核私有文件0份、自动人工确认0和收尾网络请求0。逐阶段耗时、CPU、进程峰值内存和输出字节统计已在两端复验。DGX真实完整运行重建8阶段，随后从 `RENDERING` 重跑时复用7阶段、只重建1阶段，资源字段和输出大小逐项一致。五类Agent/13工具/14调用/19产物哈希追踪、私有人工门禁确认器、80秒成片观看、最终彩排和最终录屏完整观看证据链、Web接口、安全下载和路演断言已在两端复验；默认确认保持0/5，未替参赛者关闭门禁。
