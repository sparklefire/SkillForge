...
两种模式都不会调用外部模型。默认模式复用已通过检查的安全成片，依次完成8来源摄取、142条 Evidence Catalog、13步候选 SOP、受控错误质检、局部修订、检查清单和测验。页面优先显示 N31 真实素材彩排，并明确标注 `NOT_GOLD / PROVISIONAL_ONLY`；若 N31 预处理输出不存在，才回退到无版权模拟案例。
...
- 已生成13步非Gold候选 SOP 和人工审核表，并在候选基准上完成7个严重问题到0个的局部修订彩排。
- Web Demo 已接入 N31 真实素材彩排并保留模拟回退；候选彩排数字不得当作最终 Gold 评测结果。
- 仍缺专家口述、签字授权、操作者审核和授权后的 ASR/视觉理解。
