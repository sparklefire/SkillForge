...
Web 默认监听 `0.0.0.0:7860`。页面包含上传预处理、质检问题与证据、修订前后对比、局部修订审计、手机检查清单和培训测验，并可下载最终 SOP、检查清单、测验和修订记录。下载白名单不包含原始素材。ASR 默认关闭；只有同时勾选外部处理授权时，规范化音频才会发送给 StepAudio。
...
- Web 成果区直接展示13项手机检查清单和5题培训测验，并只允许下载4类安全结构化成果；任意其他文件名返回404。
- 本机和DGX Python 3.12均已通过35项自动测试；Gold闭环、离线模式、多源/视觉接口和现场重跑烟测通过，未同步原始素材。
