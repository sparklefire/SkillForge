...
- 本机当前通过328项自动测试，DGX上一冻结版本通过320项；新增官方表单材料包与11阶段收尾链正在等待本批DGX增量复验。逐阶段耗时、CPU、进程峰值内存和输出字节统计已在两端复验。DGX真实完整运行重建8阶段，随后从 `RENDERING` 重跑时复用7阶段、只重建1阶段，资源字段和输出大小逐项一致。五类Agent/13工具/14调用/19产物哈希追踪、私有人工门禁确认器、既有10阶段提交收尾协调器、80秒成片观看和最终彩排证据链、Web接口、安全下载和路演断言已在两端复验；默认确认保持0/5，未替参赛者关闭门禁。交付配置、6段低码率预览、源文件哈希/时间轴映射和Web Range播放也已完成两端验证。DGX回环Web还实测了ASR人工修正、Evidence摘要重绑、冲突裁决和安全人工门禁。18项路演成果、真实产物阶段重跑、传输容错、可恢复工作流检查点、离线包跨文件绑定、操作者审核台、高推理语义复核、选择性重建边界、视频证据包、连续动作候选窗口、PDF结构报告、路演PPT、运行时基准、用户服务和Web白名单均已复验。
