...
- 本机当前通过135项自动测试，DGX上一部署批次通过131项；ASR人工修正批次同步复验后再更新远端数字。DGX回环Web已实测Gold五项冲突自动采用、安全冲突人工门禁和非法人工覆盖拒绝。17项路演成果、真实产物阶段重跑、传输容错、可恢复工作流检查点、离线包跨文件绑定、操作者审核台、冲突裁决、Evidence安全定位、高推理语义复核、选择性重建边界、成片SHA-256、视频证据包、连续动作候选窗口、PDF结构报告、路演PPT、运行时基准、用户服务和Web白名单均已复验。
