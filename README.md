...
Web 默认监听 `0.0.0.0:7860`。页面包含上传预处理、质检问题与证据、修订前后对比、局部修订审计、手机检查清单和培训测验，并可下载最终 SOP、检查清单、测验、A4海报和修订记录。下载白名单不包含原始素材。ASR 默认关闭；只有同时勾选外部处理授权时，规范化音频才会发送给 StepAudio。
...
- 本机已通过36项自动测试；DGX上一批35项测试及Gold闭环、离线模式、多源/视觉接口和现场重跑烟测通过，未同步原始素材。
