...
Web 默认监听 `0.0.0.0:7860`。页面包含上传预处理、五类Agent与工具追踪、三来源候选合并、质检问题与证据、修订前后对比、局部修订审计、无来源内容拒绝门禁、手机检查清单、培训测验、连续动作候选窗口、PDF结构验证、受众/语言/时长交付配置、6段低码率素材预览和80秒培训视频，并可下载最终 SOP、Agent工具追踪、候选合并报告、门禁报告、检查清单、测验、A4海报、培训视频、视频生成清单、视频证据包、连续动作候选窗口、PDF结构报告和修订记录。下载白名单不包含原始素材。ASR 默认关闭；只有同时勾选外部处理授权时，规范化音频才会发送给 StepAudio。
...
- 本机与DGX Python 3.12当前均通过145项自动测试；五类Agent/13工具/14调用/19产物哈希追踪、Web接口、安全下载和两项路演断言已在两端复验。交付配置、6段低码率预览、源文件哈希/时间轴映射和Web Range播放也已完成两端验证。DGX回环Web还实测了ASR人工修正、原始文本保留、Evidence摘要重绑、必要术语拒绝、Gold五项冲突自动采用、安全冲突人工门禁和非法人工覆盖拒绝。18项路演成果、真实产物阶段重跑、传输容错、可恢复工作流检查点、离线包跨文件绑定、操作者审核台、ASR修正、冲突裁决、Evidence安全定位、高推理语义复核、选择性重建边界、成片SHA-256、视频证据包、连续动作候选窗口、PDF结构报告、路演PPT、运行时基准、用户服务和Web白名单均已复验。
