...
if(d.source_candidate_synthesis){const c=d.source_candidate_synthesis,s=c.summary;document.querySelector('#candidate-panel').hidden=false;document.querySelector('#candidate-metrics').innerHTML=[['视频候选',s.source_candidate_counts.video],['手册候选',s.source_candidate_counts.pdf],['口述候选',s.source_candidate_counts.audio],['合并步骤',s.ordered_step_count],['多源步骤',s.multi_source_step_count],['粗/细粒度',`${s.coarse_candidate_count}/${s.fine_candidate_count}`]].map(x=>`<div class="metric"><span class="muted">${esc(x[0])}</span><strong>${esc(x[1])}</strong></div>`).join('');document.querySelector('#candidate-note').textContent='候选先按视频、手册和口述分别保存，再拆分过粗动作、合并同义或过细动作并校验依赖。S04视频不可见，因此没有把时间邻近误作视觉支持。';document.querySelector('#candidate-groups').innerHTML=c.merge_groups.slice(0,8).map(g=>`<div class="change"><b>${esc(g.step_id)} · ${esc(g.operations.join(' + '))}</b><div>${esc(g.rationale)}</div><div class="evidence">来源：${esc(g.source_types.join('、'))}｜候选：${esc(g.candidate_ids.join(', '))}｜Evidence：${esc(g.evidence_ids.join(', '))}</div></div>`).join('')}
